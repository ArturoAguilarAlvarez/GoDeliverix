...
namespace DBControl
{
    public class DBTerminosYCondiciones
    {
        ...
        public DataTable BuscaTerminosYCondiciones(string Idioma, string UidLada = "", string UidUsuario = "")
        {
            ...
            {
                query = @"select  top 1  tyc.Uid, ityc.VchUrl from TYCusuario tycu inner join TerminosYCondiciones tyc on tyc.Uid = tycu.UidTYC
                            inner join IdiomaTYC ityc on ityc.UidTYC = tyc.Uid where tycu.UidUsuario = '" + UidUsuario + "' and (ityc.VchNombre = '" + Idioma + "' or ityc.BDefault =1) and tycu.BAceptada = 1 order by tyc.DtmFechaDeCreacion desc";
            }
            return oConexion.Consultas(query);
        }
        #endregion

...
namespace DBControl
{
    public class DBOrden
    {
        ...
        {
            oConexion = new Conexion();
            string query = "select ot.UidOrden, s.Identificador,os.IntFolio, dbo.EstatusActualDeOrden(ot.UidOrden) as estatus from OrdenTarifario ot " +
                "inner join Tarifario t on t.UidRegistroTarifario = ot.UidTarifario inner join ZonaDeRecoleccion ZDR on ZDR.UidZonaDeRecolecta = t.UidRelacionZonaRecolecta " +
                "inner join Sucursales s on s.UidSucursal = ZDR.UidSucursal inner join OrdenSucursal os on os.UidRelacionOrdenSucursal = ot.UidOrden where ot.UidCodigo = '" + strcodigo + "' ";

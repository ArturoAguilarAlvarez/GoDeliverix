...
namespace DBControl
{
    public class DBTurno
    {
        ...
        {
            string query = " select count(*) as TotalOrdenes, sum(os.MTotalSucursal) as totalSucursal,sum(t.MCosto) as totalEnvio,dbo.asp_ObtenerLaCantidadDeEfectivoEnMano('" + uidTurno.ToString() + "') as Efectivo, SUM(ot.MPropina) as propina  from OrdenSucursal os   inner join OrdenTarifario ot on ot.UidOrden = os.UidRelacionOrdenSucursal inner join Tarifario t on t.UidRegistroTarifario = ot.UidTarifario  inner join OrdenRepartidor orep on orep.UidOrden = ot.UidRelacionOrdenTarifario inner join TurnoRepartidor tr on tr.UidTurnoRepartidor = orep.UidTurnoRepartidor left join (select top 1 * from LiquidacionRepartidor where UidTurnoRepartidor = '" + uidTurno.ToString() + "' order by dtmfecharegistro desc) lr on lr.UidTurnoRepartidor = tr.uidturnorepartidor    where tr.UidTurnoRepartidor = '" + uidTurno.ToString() + "' and(lr.DtmFechaRegistro is not null and orep.dtmFechaAsignacion > lr.DtmFechaRegistro or lr.DtmFechaRegistro is null) and dbo.asp_ObtenerUltimoEstatusOrdenRepartidor(orep.UidRelacionOrdenRepartidor) != '12748F8A-E746-427D-8836-B54432A38C07' and dbo.asp_ObtenerUltimoEstatusOrdenRepartidor(orep.UidRelacionOrdenRepartidor) = '7DA3A42F-2271-47B4-B9B8-EDD311F56864' ";
            oConexion = new Conexion();
            return oConexion.Consultas(query);
        }
